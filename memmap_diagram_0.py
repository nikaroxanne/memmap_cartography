...
mem.attr(bgcolor='black:green', style='filled', gradientangle='90', 
...
for i,entry  in enumerate(ivtnodename_composite_entries):
    ...
    if i < num_intermediate_entries:
        intermediate_entry_start='|<f'+ f"{current_f_index:d}" +'>...'
        current_f_index+=1
...
ivtnodename_combined=list((a,b) for a,b in final_ivtnodename_entries[i],intermediate_ivtnodename_entries[i] for i in range(num_intermediate_entries)) 
